...
void InputParams::prepare_src_map(){
    ...
    if (src_rec_file_exist && id_sim==0 && subdom_main) {
        ...
        if (swap_src_rec) {
            ...
            stdout_by_main("Do not swapping src and rec ... This may take few minutes for a large dataset (only regional events will be processed)\n");
            do_not_swap_src_rec(src_map_all, rec_map_all, data_map_all);
        }

        process_tele_data(src_map_tele,rec_map_tele,data_map_tele);
                    ...
                    for (const auto& data_ori : v_data){
                        ...
                        // common source differential traveltime
                            ...
                            if (get_is_srcrec_swap()) { // do reverse swap
                                ...
                                DataInfo data1 = get_data_rec_pair(data_map_all[name_src][name_rec1]);
                                DataInfo data2 = get_data_rec_pair(data_map_all[name_src][name_rec2]);
                                
                                cs_dif_travel_time = data1.travel_time - data2.travel_time;
                            }
...
void InputParams::check_data_map(){
    for (int i = 0; i < n_sims; i++){   
        if(id_sim == i){
            std::cout << "id_sim: " << id_sim << ", checking data map ..." << std::endl;
            std::cout << "size of src: " << data_map.size() << std::endl;
            for(auto it_src = data_map.begin(); it_src != data_map.end(); it_src++){
                std::cout << "src: " << it_src->first << std::endl;
                std::cout << "size of rec: " << it_src->second.size() << std::endl;
                for(auto it_rec = it_src->second.begin(); it_rec != it_src->second.end(); it_rec++){
                    std::cout << "rec: " << it_rec->first << std::endl;
                    std::cout << "size of data: " << it_rec->second.size() << std::endl;
                    for(const auto& data: it_rec->second){
                        if (data.is_src_rec){
                            std::cout   << "key: it_src, it_rec: " << it_src->first << ", " << it_rec->first
                                        << ", source name: "       << data.name_src
                                        << ", receiver name: "     << data.name_rec
                                        << ", obs absolute traveltime: " << data.travel_time_obs
                                        << ", syn traveltime: " << data.travel_time
                                        << std::endl;
                        } else if (data.is_rec_pair){
                            std::cout   << "key: it_src, it_rec: " << it_src->first << ", " << it_rec->first
                                        << ", source name: "                                << data.name_src
                                        ...
                                        << ", obs common source differential traveltime: "        << data.cs_dif_travel_time_obs
                                        << ", syn traveltime: " << data.travel_time
                                        << std::endl;
                        } else if (data.is_src_pair){
                            std::cout   << "key: it_src, it_rec: " << it_src->first << ", " << it_rec->first
