...
//             // get source info
//             SrcRec& src = IP.get_src_point_nv(src_id);
...
//                 IP.allocate_memory_tele_boundaries(loc_I, loc_J, loc_K, src_id,grid.i_first(), grid.i_last(), grid.j_first(), grid.j_last(), grid.k_first());
...
void prepare_teleseismic_boundary_conditions(InputParams& IP, Grid& grid, IO_utils& io) {
    ...
    if (subdom_main) {
        for(long unsigned int i_src = 0; i_src < IP.src_names_this_sim_tele.size(); i_src++){   
            // int id_sim_src = IP.src_ids_this_sim[i_src];
            std::string name_sim_src = IP.src_names_this_sim_tele[i_src];
            // get source info
            SrcRecInfo& src = IP.get_src_point_nv(name_sim_src);
