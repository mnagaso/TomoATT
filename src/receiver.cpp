    ...
    if(subdom_main){
        for (auto& data : IP.data_info_smap[name_sim_src]){     // loop all data related to this source
            if (data.is_src_rec){   // absolute traveltime
                ...
                IP.rec_list_nv[name_rec].adjoint_source += (syn_time - obs_time)*data.data_weight;
                allsum_obj += my_square(syn_time - obs_time)*data.data_weight;
                allsum_misfit += my_square(syn_time - obs_time);
                ...
                if (name_sim_src == name_src1){
                    ...
                    IP.rec_list_nv[name_rec].adjoint_source += (syn_dif_time - obs_dif_time)*data.data_weight;
                    allsum_obj += my_square(syn_dif_time - obs_dif_time)*data.data_weight;
                    allsum_misfit += my_square(syn_dif_time - obs_dif_time);
                ...
                IP.rec_list_nv[name_rec1].adjoint_source += (syn_dif_time - obs_dif_time)*data.data_weight;
                IP.rec_list_nv[name_rec2].adjoint_source -= (syn_dif_time - obs_dif_time)*data.data_weight;
                allsum_obj += my_square(syn_dif_time - obs_dif_time)*data.data_weight;
                allsum_misfit += my_square(syn_dif_time - obs_dif_time);
            }
        }
